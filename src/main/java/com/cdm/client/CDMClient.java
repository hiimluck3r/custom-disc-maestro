...
import com.cdm.client.screen.SleeveScreen;
...
/**
 * Client-only bootstrap. Never loaded on a dedicated server, so client-only audio/render code
 * (JFileChooser, dynamic textures, BEWLRs) is wired in from here in later milestones.
 */
...
public class CDMClient {
    ...
    static void onClientSetup(FMLClientSetupEvent event) {
        event.enqueueWork(() -> {
            ...
            ItemProperties.register(ModItems.SLEEVE.get(), SLEEVE_LOOK,
                    (stack, level, entity, seed) -> {
                        SleeveDesign d = stack.get(ModComponents.SLEEVE_DESIGN.get());
                        return d == null ? 0.0F : d.sticker() / (float) SleeveDesign.STICKER_COUNT;
                    });
            // Stencil reuses the sleeve_look predicate so its sticker shows through in the inventory.
            ItemProperties.register(ModItems.SLEEVE_STENCIL.get(), SLEEVE_LOOK,
                    (stack, level, entity, seed) -> {
                        SleeveDesign d = stack.get(ModComponents.SLEEVE_DESIGN.get());
                        return d == null ? 0.0F : d.sticker() / (float) SleeveDesign.STICKER_COUNT;
                    });
    ...
    static void onRegisterItemColors(RegisterColorHandlersEvent.Item event) {
        ...
        event.register((stack, tint) -> {
            ...
            return switch (tint) {
                case 0 -> 0xFF000000 | d.bgColor();
                case 1 -> 0xFF000000 | d.stickerColor();
                default -> 0xFFFFFFFF;
            };
        }, ModItems.SLEEVE.get());
    ...
    static void onRegisterScreens(RegisterMenuScreensEvent event) {
        ...
        event.register(ModMenus.SLEEVE.get(), SleeveScreen::new);
    }
}
